...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Marketcoin " + FormatFullVersion();
...
//here we should add the known ip addresses on the network
unsigned int pnSeed[] =
{
  0x0026cf23,0x4d3dcb23,0x1c05c923
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Marketcoin is probably already running."), addrBind.ToString().c_str());
        else
